...
- (void)playWithUrl:(NSURL *)url showView:(UIView *)showView
{
    ...
    if (IOS_VERSION < 7.0 || ![str hasPrefix:@"http"]) {
        ...
        if (IOS_VERSION > 9.0) {
            self.currentPlayerItem.canUseNetworkResourcesForLiveStreamingWhilePaused = YES;
        }
        if (!self.player) {
            self.player = [AVPlayer playerWithPlayerItem:self.currentPlayerItem];
        } else {
...
- (void)seekToTime:(CGFloat)seconds
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"DragSlider" object:nil];
