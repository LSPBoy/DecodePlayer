...
-(void)URLSession:(NSURLSession *)session dataTask:(nonnull NSURLSessionDataTask *)dataTask didReceiveData:(nonnull NSData *)data{
    ...
    if (self.reciveDataArray.count == 1)
    {
        _tempFileSize -= 3;
    }
    ...
    while (1) {
        ...
        if (self.reciveDataArray.count)
        {
            ...
            [self.decodeHelp decryptionData:data completion:^(NSData *currentData, NSData *decodeData) {
                
                [self.stream write:decodeData.bytes maxLength:decodeData.length];
